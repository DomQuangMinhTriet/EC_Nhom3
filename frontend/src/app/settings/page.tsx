...
const schema = z.object({
  ...
  confirm: z.string(),
}).refine((values) => values.password === values.confirm, { path: ["confirm"], message: "Mật khẩu xác nhận chưa khớp" });
...
function SettingsContent() {
  ...
  async function submit(values: Values) {
    try {
      await changePassword.mutateAsync({ currentPassword: values.currentPassword, newPassword: values.password });
  ...
  return (
    <CustomerShell active="/settings">
      <PageHeader title="Cài đặt" subtitle="Tùy chỉnh bảo mật và thông báo tài khoản." />
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-brand-sm">
        <h2 className="font-extrabold text-slate-900">Thông báo</h2>
        <div className="mt-4 space-y-4 text-xs">
          <Toggle label="Nhận thông báo ưu đãi mới" />
          <Toggle label="Nhắc voucher sắp hết hạn" />
          <Toggle label="Email xác nhận đơn hàng" />
        </div>
      </section>
      <form onSubmit={handleSubmit(submit)} className="mt-5 rounded-xl border border-slate-200 bg-white p-6 shadow-brand-sm">
        <h2 className="font-extrabold text-slate-900">Đổi mật khẩu</h2>
...
function Toggle({ label }: { label: string }) {
  return (
    <label className="flex items-center justify-between text-slate-700">
      <span>{label}</span>
      <input defaultChecked type="checkbox" className="h-4 w-4 accent-[#4F46E5]" />
    </label>
  );
}
