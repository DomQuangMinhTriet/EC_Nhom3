...
import { useCart, useRemoveCartItem } from "@/hooks/queries/use-cart";

const checkoutSchema = z.object({ name: z.string().min(2, "Nhập họ tên người nhận"), email: z.string().email("Nhập email hợp lệ"), phone: z.string().regex(/^(0|\+84)\d{9,10}$/, "Số điện thoại không hợp lệ"), payment: z.enum(["momo", "vnpay", "bank"]) });
...
export function CheckoutScreen() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<CheckoutValues>({ resolver: zodResolver(checkoutSchema), defaultValues: { payment: "momo" } });

  const items = cartQuery.data?.items ?? [];
  const total = items.reduce((sum, item) => sum + Number(item.unitPrice) * item.quantity, 0);

  async function submit() {
    // Backend chưa có API tạo đơn hàng/thanh toán thật (chỉ có schema orders/payments,
    // chưa có module xử lý) — đây vẫn là luồng mô phỏng, chỉ dọn giỏ hàng thật sau khi
    // "thanh toán" để tránh để lại dữ liệu giỏ hàng cũ gây nhầm lẫn.
    await Promise.all(items.map((item) => removeItem.mutateAsync(item.cartItemId)));
    const code = `ECO-${Math.floor(100000 + Math.random() * 900000)}`;
    router.push(`/order-confirmation/${code}`);
  }
        ...
        {!items.length ? (
          ...
          <form onSubmit={handleSubmit(submit)} className="mt-7 grid gap-6 lg:grid-cols-[1fr_330px]">
            <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-brand-sm">
              ...
              <div>
                ...
                <div className="mt-3 space-y-2">
                  {[["momo", "Ví MoMo"], ["vnpay", "VNPay"], ["bank", "Chuyển khoản ngân hàng"]].map(([value, label]) => (
            ...
            <aside className="h-fit rounded-xl border border-slate-200 bg-white p-5 shadow-brand-md">
              ...
              <Button type="submit" size="lg" fullWidth disabled={isSubmitting}>{isSubmitting ? "Đang tạo đơn..." : "Xác nhận thanh toán"}</Button>
              <p className="mt-3 text-center text-[11px] text-slate-400">Đây là luồng thanh toán mô phỏng — backend chưa có API đặt hàng/thanh toán thật.</p>
            </aside>
          </form>
        )}
      </div>
