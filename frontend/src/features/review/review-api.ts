...
export async function createReview(voucherCodeId: string, rating: number, comment: string) {
  const res = await apiClient<{ data: Review }>("/customers/me/reviews", {
    method: "POST",
    headers: authHeaders(),
...
export async function editReview(reviewId: string, rating: number, comment: string) {
  const res = await apiClient<{ data: Review }>(`/customers/me/reviews/${reviewId}`, {
    method: "PUT",
    headers: authHeaders(),
...
export async function getVoucherReviews(voucherProductId: string) {
  const res = await apiClient<{ data: VoucherReviews }>(`/vouchers/${voucherProductId}/reviews`, { headers: authHeaders() });
  return res.data;
}

export async function changeReviewStatus(reviewId: string, status: Extract<ReviewStatus, "hidden" | "deleted">) {
  const res = await apiClient<{ data: Review }>(`/admin/reviews/${reviewId}/status`, {
    method: "PATCH",
    headers: authHeaders(),
